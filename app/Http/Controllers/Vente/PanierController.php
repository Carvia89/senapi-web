...
class PanierController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('dappro.bur-ventes.cmdInterne.form',
            compact(
                'options',
                'kelasis',
                'category_cmd',
    ...
    public function show(Request $request)
    {
        ...
        // Générer le PDF
        $libelle = $data->first()->libelle;
        $totalQteCmdee = $data->sum('qte_cmdee');
        $dateDuJour = now()->format('d/m/Y');

        $pdf = PDF::loadView('dappro.bur-ventes.Reporting.EtatBesoin', [
            'data' => $data,
            'commandeLibelle' => $libelle,
            'totalQteCmdee' => $totalQteCmdee,
            'numeroCommande' => $num_cmd,
            'dateDuJour' => $dateDuJour,
        ]);

        // Définir le nom du fichier PDF
        $pdfFileName = 'EtatBesoin_' . $num_cmd . '.pdf';
    ...
    public function edit($id)
    {
        ...
        return view('dappro.bur-ventes.cmdInterne.edit', [
            'options' => $options,
            'kelasis' => $kelasis,
            'category_cmd' => "Interne",
            'num_cmd' => $panier->num_cmd,
            'clients' => $clients,
            'enregistrements' => Panier::orderBy('created_at', 'asc')->paginate(85),
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('admin.commande-Interne.create')->with([
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('admin.commande-Interne.create')->with('success', 'Article supprimé avec succès.');
    }
}
