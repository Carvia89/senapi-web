...
<body>
    ...
    <div class="title" style="text-align: center; font-size: 15px; font-weight: bold; font-family: Arial, sans-serif;">
        <p>RAPPORT SUR LES DEPENSES</p>
    </div><br>

    <div class="under-title">
        <p>Dossier : {{ $dossier->designation ?? '-' }}</p>
        <p>Imputation : {{ $imputation->imputation ?? '-' }}</p>
        <p>Rubrique d'imputation : {{ $referenceImputation->designation ?? '-' }}</p>
        ...
        <tbody>
            @foreach ($depenses as $depense)
                <tr>
                    ...
                    <td style="text-align: right;">{{ number_format($depense->montant_depense, 2, ',', ' ') }}</td>
                </tr>
            @endforeach
