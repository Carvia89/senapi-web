...
<body>
    ...
    <div class="additional-info">
        <p> <strong>Article : {{ $article_id }}</strong> </p>
        <p> <strong>Période : Du {{ \Carbon\Carbon::parse($date_debut)->format('d/m/Y') }} au {{ \Carbon\Carbon::parse($date_fin)->format('d/m/Y') }} </strong> </p>
    </div>
        ...
        <tbody>
            ...
            @foreach ($entrees as $index => $entree)
                <tr>
                    ...
                    <td style="text-align: right">{{ number_format($entree->quantite, 0, ',', ' ') }}</td>
                    ...
                    @php
                        ...
                        if ($index == 0) {
                            $stockTotal += $entree->quantite; // Pour la première ligne
                        } else {
                            $stockTotal = $entree->quantite + $previousSolde; // Pour les lignes suivantes
                        }
                    @endphp
                    ...
                    @if (isset($sorties[$index]))
                        ...
                        <td style="text-align: right">{{ number_format($sorties[$index]->quantiteLivree, 0, ',', ' ') }}</td>
                        @php
                            $solde = $stockTotal - $sorties[$index]->quantiteLivree;
                        @endphp
